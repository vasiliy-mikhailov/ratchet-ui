...
function lampStyle(lit: boolean, reached: boolean, colour: string): Style {
  if (lit) {
    return {
      ...LAMP,
 ...
 */
export function Lamp({ lit, reached, colour, label }: LampProps) {
  return <span role="img" aria-label={label} title={label} style={lampStyle(lit, reached, colour)} />
}
