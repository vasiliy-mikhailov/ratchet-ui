...
const SAID: Style = { marginLeft: '8px', fontSize: '12.5px', color: 'var(--text-secondary)' }
 ...
 * wanted one shown.
 */
export function KeyStatus({ keyed, keySource, whenAbsent }: KeyStatusProps) {
  return (
    <div style={{ margin: '0 0 12px' }}>
      <Pill tone={keyed ? 'good' : 'alarm'}>{keyed ? 'key set' : 'no key'}</Pill>
      <span style={SAID}>
        {keyed ? `the agents are using the key from ${keySource}` : whenAbsent}
      </span>
    </div>
  )
}
