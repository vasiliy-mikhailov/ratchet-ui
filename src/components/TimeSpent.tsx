...
const COUNT: Style = { color: 'var(--text-tertiary)', fontSize: '11px' }

/** Nothing to report reads as nothing to report, in the colour of a thing that is absent. */
const NOTHING: Style = { color: 'var(--text-tertiary)' }
 ...
 */
export function TimeSpent({ ms, events }: TimeSpentProps) {
  if (ms === null) {
    return <span style={NOTHING}>{'—'}</span>
  }
  return (
    <>
      <span>{duration(ms)}</span>
      <div style={COUNT}>{events.toLocaleString()} event(s)</div>
    </>
  )
}
