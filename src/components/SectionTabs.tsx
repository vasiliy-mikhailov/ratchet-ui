 ...
 */
export function SectionTabs({ tabs, trailing, label }: SectionTabsProps) {
  return (
    <nav style={BAR} aria-label={label}>
      {tabs.map((tab) => (
        <a
          key={tab.href}
          href={tab.href}
      ...
      {(trailing ?? []).map((tab, index) => (
        <a
          key={tab.href}
          href={tab.href}
          aria-current={tab.current ? 'page' : undefined}
          style={index === 0 ? { ...tabStyle(tab.current), marginLeft: 'auto' } : tabStyle(tab.current)}
        >
          {tab.label}
        </a>
      ))}
