...
/**
 * The reading half of a check, kept in one place because every rule needs the same two questions
 * answered: is the key there at all, and is what is under it the right kind of thing.
 */
 ...
 */
function absent(bag: Bag, key: string): boolean {
  return !(key in bag) || bag[key] === undefined
}

function root(value: unknown, at: string, what: string): Problem[] {
  return isBag(value) ? [] : [{ path: at, says: `expected ${what} object, got ${kindOf(value)}` }]
}

function str(bag: Bag, key: string, at: string): Problem[] {
  if (absent(bag, key)) return [{ path: `${at}.${key}`, says: 'is missing, expected a string' }]
  const value = bag[key]
 ...
 */
function nullableStr(bag: Bag, key: string, at: string): Problem[] {
  if (absent(bag, key)) {
 ...
 */
function num(bag: Bag, key: string, at: string): Problem[] {
  if (absent(bag, key)) return [{ path: `${at}.${key}`, says: 'is missing, expected a number' }]
  const value = bag[key]
...
function count(bag: Bag, key: string, at: string): Problem[] {
...
function optional(bag: Bag, key: string, at: string, check: (b: Bag, k: string, a: string) => Problem[]): Problem[] {
  if (absent(bag, key)) return []
  return check(bag, key, at)
}

function arrayAt(bag: Bag, key: string, at: string): { items: unknown[] } | { problems: Problem[] } {
  if (absent(bag, key)) {
 ...
 */
function safely(at: string, run: () => Problem[]): Problem[] {
  try {
    return run()
  } catch (thrown) {
...
export function checkWorkItem(value: unknown, at = 'item'): Problem[] {
  return safely(at, () => {
...
export function checkWorkItems(value: unknown, at = 'items'): Problem[] {
  return safely(at, () => {
    if (!Array.isArray(value)) {
...
export function checkRecordEvent(value: unknown, at = 'event'): Problem[] {
  return safely(at, () => {
...
export function checkItemDetail(value: unknown, at = 'detail'): Problem[] {
  return safely(at, () => {
    ...
    if ('problems' in events) return [...problems, ...events.problems]
    return [
      ...problems,
...
export function checkFinding(value: unknown, at = 'finding'): Problem[] {
  return safely(at, () => {
    ...
    if ('problems' in items) return [...problems, ...items.problems]
    return [
      ...problems,
      ...items.items.flatMap((id, i) =>
        typeof id === 'string'
          ? []
...
function checkNavItem(value: unknown, at: string): Problem[] {
 ...
 * one extra rule.
 */
export function checkManifest(value: unknown, at = 'manifest'): Problem[] {
  return safely(at, () => {
 ...
 */
export function checkHealth(value: unknown, at = 'health'): Problem[] {
  return safely(at, () => {
    const bad = root(value, at, 'a health response')
    if (bad.length > 0) return bad
    const bag = value as Bag
    if (absent(bag, 'ok')) return [{ path: `${at}.ok`, says: 'is missing, expected true or false' }]
    if (bag['ok'] === true) return str(bag, 'version', at)
    if (bag['ok'] === false) return str(bag, 'why', at)
